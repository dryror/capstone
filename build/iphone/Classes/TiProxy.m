...
//Rare exceptions to indicate a bug in the _ltema code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_ltema = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_ltema]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.ltema.module"
